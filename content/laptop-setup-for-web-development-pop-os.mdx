...
1. Update `git` and create a config file.
...
2. Install `zsh`.
   ...
   - Install `zplug` (zsh plugin manager) and create a `zshrc` config file.
...
3. Install `tmux`.
   ...
   - Install `tpm` (TMUX plugin manager) and create a `tmux` config file.
...
6. Install docker-compose V1.

   **Note:** Docker Compose V1 stopped receiving updates and is no longer supported. The Docker installation above already includes V2.
